...
const config = {
  ...
  cache: true
    ...
    const DiffLineComponent = ({ line, index }: { line: DiffLine; index: number }) => {
        ...
        return (
            <div key={index} className={cn("flex items-start gap-2 px-3 py-1 font-mono text-sm", getLineStyle())}>
        ...
        return (
            <div className="grid grid-cols-2 gap-4">
                <div className="space-y-2">
                    <h3 className="font-semibold text-sm text-red-600 dark:text-red-400">Original</h3>
                                ...
                                className={cn(
                                    ...
                                    i >= originalLines.length
                                        ...
                                        : originalLines[i] !== (modifiedLines[i] || "")
                                            ? "bg-red-50 border-l-4 border-l-red-500 text-red-800 dark:bg-red-950 dark:text-red-200"
                                            : "bg-gray-50 border-l-4 border-l-gray-300 text-gray-700 dark:bg-gray-900 dark:text-gray-300",
                                )}
                            >
                                <span className="w-8 text-right text-xs opacity-60">{i + 1}</span>
                                <span className="flex-1 whitespace-pre-wrap break-all">{originalLines[i] || ""}</span>
                            </div>
                        ))}
                    </div>
                </div>
                <div className="space-y-2">
                    <h3 className="font-semibold text-sm text-green-600 dark:text-green-400">Modified</h3>
                    <div className="border rounded-lg overflow-hidden">
                        {Array.from({ length: maxLines }, (_, i) => (
                            <div
                                key={i}
                                className={cn(
                                    ...
                                    i >= modifiedLines.length
                                        ...
                                        : modifiedLines[i] !== (originalLines[i] || "")
                                            ? "bg-green-50 border-l-4 border-l-green-500 text-green-800 dark:bg-green-950 dark:text-green-200"
                                            : "bg-gray-50 border-l-4 border-l-gray-300 text-gray-700 dark:bg-gray-900 dark:text-gray-300",
                                )}
                            >
                                <span className="w-8 text-right text-xs opacity-60">{i + 1}</span>
                                <span className="flex-1 whitespace-pre-wrap break-all">{modifiedLines[i] || ""}</span>
                            </div>
                        ))}
                    </div>
                </div>
        ...
        <div className="space-y-6">
            <Card>
                <CardHeader>
                    ...
                    <CardDescription>
                        Compare two pieces of text and see the differences highlighted. Perfect for code reviews, configuration
                        comparisons, or tracking changes.
                    </CardDescription>
                </CardHeader>
                        ...
                        <Tabs defaultValue="unified" className="w-full">
                            ...
                            <TabsContent value="unified" className="mt-4">
                                <div className="border rounded-lg overflow-hidden max-h-[600px] overflow-y-auto">
                                    {computeDiff.length > 0 ? (
                                        computeDiff.map((line, index) => <DiffLineComponent key={index} line={line} index={index} />)
                                    ) : (
                                        <div className="p-4 text-center text-muted-foreground">No differences found</div>
                                    )}
                                </div>
                            </TabsContent>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <div className="space-y-2 text-sm">
                                ...
                                <div className="flex items-center gap-2">
                                    <div className="w-4 h-4 bg-gray-200 border-l-4 border-gray-300 rounded-sm"></div>
